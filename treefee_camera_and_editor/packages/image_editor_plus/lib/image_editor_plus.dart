    ...
    return Theme(
      ...
      child: Scaffold(
              ...
              onPressed: () async {
                try {
                  ...
                  final String _apiUrl =
                      'http://api2.treefe.com:8084/postgres/api/s3/uploadfiles';
                  final String _authorizationToken =
                      'Bearer ${widget.accessToken}';

                  Future<List<Map<String, dynamic>>> getSignedUrls(
                    ...
                    try {
                      final response = await _dio.post(
                        ...
                        data: jsonEncode(files),
                      );

                      if (response.statusCode == 200) {
                        return List<Map<String, dynamic>>.from(response.data);
                      } else {
                        throw Exception('Failed to get signed URLs');
                      }
                    } catch (e) {
                      print(e);
                      throw Exception('Failed to get signed URLs');
                  ...
                  Future<List<String>> uploadImages(
                    ...
                    List<Map<String, dynamic>> files =
                      ...
                      return {
                        "extension": "jpg",
                        "fileName":
                            "image_${DateTime.now().microsecondsSinceEpoch}_$index",
                        "fileSize": entry.value.length / (1024 * 1024),
                      };
                    }).toList();

                    try {
                      List<Map<String, dynamic>> signedUrls =
                          await getSignedUrls(files);

                      await Future.wait(signedUrls.asMap().entries.map((entry) {
                        int index = entry.key;
                        return uploadImageToS3(
                            entry.value['url'], images[index]);
                      }).toList());

                      return signedUrls
                          .map<String>((e) => e['url'].split("?")[0])
                          .toList();
                    } catch (e) {
                      print(e);
                  ...
                  List<String> _urls = await uploadImages(
                      images.map<Uint8List>((e) => e.bytes).toList());
                  await _channel.invokeMethod('sendDataToNative', _urls);
        ...
        body: PageView(
            ...
            children: images
                .map<Widget>((image) => Stack(children: [
                      GestureDetector(
                        onTap: () async {
            ...
            IconButton(
              ...
              onPressed: () async {
                ...
                if ((widget.outputFormat & 0x1) == o.OutputFormat.json) {
                  ...
                  if (widget.openedFrom != 'MultiImageEditor') {
                    // showDialog(
                    //   context: context,
                    //   builder: (ctx) {
                    ...
                    //           content: const Text("From here onwards, your React Native code will start, which will receive all these processed images"),
                    //           actions: [
                    //             TextButton(
                    //               child: Text(savingImage ? "Saving..." : "Save Images"),
                    //               onPressed: () async {
                    //                 _setState(() {
                    //                   savingImage = true;
                    //                 });
                    //                 await ImageGallerySaver.saveImage(editedImageBytes!);
                    //
                    //                 _setState(() {
                    //                   savingImage = false;
                    //                 });
                    ...
                    //             TextButton(
                    //               style: ButtonStyle(
                    //                   backgroundColor: MaterialStateProperty.all(Colors.purple.withOpacity(0.1))
                    //               ),
                    ...
                    try {
                      final Dio _dio = Dio();
                      final String _apiUrl =
                          'http://apibknd.treefe.com:8084/postgres/api/s3/uploadfiles';
                      final String _authorizationToken =
                          'Bearer ${widget.accessToken}';
                      ...
                      Future<List<String>> uploadImages(
                        ...
                        try {
                          ...
                          await Future.wait(
                              signedUrls.asMap().entries.map((entry) {
                            int index = entry.key;
                            return uploadImageToS3(
                                entry.value['url'], images[index]);
                          }).toList());
                      ...
                      List<String> _urls =
                          await uploadImages([editedImageBytes!]);
                      await _channel.invokeMethod('sendDataToNative', _urls);
                      Navigator.pop(context);
                      await _channel.invokeMethod('closeFlutterActivity');
                    } catch (e) {
